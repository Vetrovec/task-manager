...
export default function WorkplacesGetLayout({
  ...
  return (
    <div className="flex gap-4">
      <div className="flex flex-col w-28 h-full py-4 gap-2 bg-blue-500 rounded-xl">
        <Link href={`/workplaces/get?id=${id}`}>
          <div className="text-center text-white">Dashboard</div>
        </Link>
        <Link href={`/workplaces/get/history?id=${id}`}>
          <div className="text-center text-white">History</div>
        </Link>
        <Link href={`/workplaces/get/payroll?id=${id}`}>
          <div className="text-center text-white">Payrolls</div>
        </Link>
        <Link href="/workplaces/list">
          <div className="text-center text-white">Exit</div>
        </Link>
      </div>
